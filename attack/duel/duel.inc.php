...
function duel_info($duel_id){
  ...
  return mysql_fetch_array(mysql_query("SELECT * FROM `duel` WHERE id='".$duel_id."'"));
}
...
function remove_duel($duel_id){
  ...
  mysql_query("DELETE FROM `duel` WHERE `id`='".$duel_id."'");
}
...
function duel_one_dead($duel_info,$you,$pokemon_info,$opponent_info,$txt){
    ...
    if(!empty($pokemonid)){  
      ...
      if($used_info['leven'] > 0){
        ...
        if($used_info['level'] < 100){
          ...
          mysql_query("UPDATE `pokemon_speler_gevecht` SET `exp`=`exp`+'".$ret['exp']."', `totalexp`=`totalexp`+'".$ret['exp'] ."', `attack_ev`=`attack_ev`+'".$opponent_info['effort_attack']."', `defence_ev`=`defence_ev`+'".$opponent_info['effort_defence']."', `speed_ev`=`speed_ev`+'".$opponent_info['effort_speed']."', `spc.attack_ev`=`spc.attack_ev`+'".$opponent_info['effort_spc.attack']."', `spc.defence_ev`=`spc.defence_ev`+'".$opponent_info['effort_spc.defence']."', `hp_ev`=`hp_ev`+'".$opponent_info['effort_hp']."' WHERE `id`='".$pokemonid."'");

          //Check if the Pokemon is traded
          if($used_info['trade'] == 1) $ret['bericht'] .= $used_info['naam_goed']." ".$txt['recieve']." ".$ret['exp'] ." ".$txt['exp_points'].".<br />";
          else $ret['bericht'] .= $used_info['naam_goed']." ".$txt['a_boosted']." ".$ret['exp'] ." ".$txt['exp_points'].".<br />";
        }
      }
      else $aantal -= 1;
    }
  ...
  mysql_query("UPDATE `duel` SET `".$you."_used_id`=',".$pokemon_info['id'].",' WHERE `id`='".$duel_info['id']."'");
...
function start_attack($duel){
  //Check who can start
  $uitdager = mysql_fetch_array(mysql_query("SELECT pw.naam, pw.wild_id, psg.user_id, ps.id, ps.speed FROM pokemon_speler AS ps INNER JOIN pokemon_speler_gevecht AS psg ON psg.id = ps.id INNER JOIN pokemon_wild AS pw On ps.wild_id = pw.wild_id WHERE psg.user_id='".$_SESSION['id']."' AND psg.leven>'0' AND ps.ei='0' ORDER BY ps.opzak_nummer ASC LIMIT 1"));
  $tegenstander = mysql_fetch_array(mysql_query("SELECT pw.naam, pw.wild_id, psg.user_id, ps.speed FROM pokemon_speler AS ps INNER JOIN pokemon_speler_gevecht AS psg ON psg.id = ps.id INNER JOIN pokemon_wild AS pw On ps.wild_id = pw.wild_id WHERE psg.id='".$duel['t_pokemonid']."'"));
  ...
  //Save 
  mysql_query("UPDATE `duel` SET `u_pokemonid`='".$duel_info['u_pokemonid']."', `u_used_id`=',".$duel_info['u_pokemonid'].",', `laatste_beurt`='".$duel_info['laatste_beurt']."', `volgende_beurt`='".$duel_info['volgende_beurt']."' WHERE `id`='".$duel['id']."'");
  //Update Both pokedexes
  mysql_query("UPDATE gebruikers SET `pok_gezien`=concat(pok_gezien,',".$uitdager['wild_id']."') WHERE user_id='".$tegenstander['user_id']."'");
  mysql_query("UPDATE gebruikers SET `pok_gezien`=concat(pok_gezien,',".$tegenstander['wild_id']."') WHERE user_id='".$uitdager['user_id']."'");
  return;
}
?>