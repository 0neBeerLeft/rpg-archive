...
if ((isset($_GET['pokemon_info_name'])) && (isset($_GET['computer_info_name'])) && (isset($_GET['aanval_log_id'])) && (isset($_GET['sid']))) {
    ...
    if ($aanval_log['laatste_aanval'] == "trainer_wissel") {
        $new_computer = mysql_fetch_array(mysql_query("SELECT pokemon_wild.naam, pokemon_wild.wild_id, pokemon_wild_gevecht.id, pokemon_wild_gevecht.levenmax, pokemon_wild_gevecht.leven, pokemon_wild_gevecht.speed FROM pokemon_wild INNER JOIN pokemon_wild_gevecht ON pokemon_wild.wild_id = pokemon_wild_gevecht.wildid WHERE `aanval_log_id`='" . $_GET['aanval_log_id'] . "' AND `leven`>'0' ORDER BY rand() limit 1"));
        ...
        //Update aanval_log
        mysql_query("UPDATE `aanval_log` SET `laatste_aanval`='" . $lastmove . "', `tegenstanderid`='" . $new_computer['id'] . "' WHERE `id`='" . $aanval_log['id'] . "'");
        //Save Computer As Seen in Pokedex
        update_pokedex($new_computer['wild_id'], '', 'zien');

        echo $message . " | " . $new_computer['naam'] . " | " . $new_computer['leven'] . " | " . $new_computer['levenmax'] . " | " . $refresh . " | " . $aanval_log['tegenstanderid'] . " | " . $new_computer['wild_id'];
