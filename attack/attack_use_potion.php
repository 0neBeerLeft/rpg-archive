...
if ((isset($_GET['item'])) && (isset($_GET['sid'])) && (isset($_GET['aanval_log_id'])) && (isset($_GET['option_id'])) && (isset($_GET['potion_pokemon_id'])) && (isset($_GET['computer_info_name']))) {
    ...
    $player_item_info = mysql_fetch_array(mysql_query("SELECT `Potion`, `Super potion`, `Hyper potion`, `Full heal`, `Revive`, `Max revive` FROM `gebruikers_item` WHERE `user_id`='" . $_SESSION['id'] . "'"));
    //Load pokeball info
    ...
    elseif ($item_info['wat'] != "potion") $message = $txt['potion_have'];
    //Check if you have that pokeball
    elseif ($player_item_info[$item_info['naam']] <= 0) $message = $txt['potion_amount'] . $_GET['item'] . ".";
    //Pokemon has full life
    elseif ($pokemon_info['leven'] === $pokemon_info['levenmax']) $message = $pokemon_info['naam_goed'] . $txt['potion_life_full'];
    //Check if the fight is finished yet
    elseif ($aanval_log['laatste_aanval'] == "klaar") $message = $txt['dead_1'] . $_GET['computer_info_name'] . $txt['dead_2'];
    //Check if it is not your turn
    elseif ($aanval_log['laatste_aanval'] == "pokemon") $message = $_GET['computer_info_name'] . $taal['attack']['general']['lastattack'];
    //Use Potion
    else {
        //Effect Info
        ...
        switch ($item_info['apart']) {
            ...
            case "nee":
                ...
                if ($pokemon_info['leven'] <= 0) $message = $txt['potion_life_zero_1'] . $pokemon_info['naam_goed'] . $txt['potion_life_zero_2'];
                //pokemon is not dead
                else {
        ...
        $good = 1;
        //Save new life
        mysql_query("UPDATE `pokemon_speler_gevecht` SET `leven`='" . $new_life . "', `effect`='" . $pokemon_effect . "' WHERE `id`='" . $pokemon_info['id'] . "'");
        //Update aanval log
        mysql_query("UPDATE `aanval_log` SET `laatste_aanval`='pokemon' WHERE `id`='" . $aanval_log['id'] . "'");
        //Remove Potion
        mysql_query("UPDATE `gebruikers_item` SET `" . $item_info['naam'] . "`='" . $new_amount . "' WHERE `user_id`='" . $_SESSION['id'] . "'");
    }
    ...
    echo $message . " | " . $good . " | " . $info_potion_left . " | " . $_GET['option_id'] . " | " . $item_info['naam'] . " | Potion | " . $new_life . " | " . $pokemon_info['levenmax'] . " | " . $pokemon_infight . " | " . $pokemon_info['opzak_nummer'] . " | " . $pokemon_info['naam_goed'] . " | " . $pokemon_info['id'];
}
?>