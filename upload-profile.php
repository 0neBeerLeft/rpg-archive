...
function generate_resized_image(){
    ...
    $postvars = array(
        ...
        "image_max_width"    => (int)133, "image_max_height"   => (int)208
    );
...
if(isset($_GET["do"])){
    if($_GET["do"] == "upload"){
        ...
        if($mysql){
            echo "<div class='green'>Profiel foto aangepast.</div>";
            echo "<meta http-equiv='refresh' content='3;url=?page=account-options&category=picture'>";
        }
        if (!$mysql) {
            //handle error
...
function saveToDb($filename)
{
    ...
    $q = "UPDATE `gebruikers` SET `profielfoto`=:file WHERE `user_id`=:userid";
    $st = $db->prepare($q);
